...
class NewShoppingListsController extends GetxController {
  ...
  final RxList<TextEditingController> itemControllers = <TextEditingController>[].obs;
  ...
  void saveList() {
    final items = itemControllers.map((c) => c.text).where((text) => text.trim().isNotEmpty).toList();
